...
18. [개인정보 보호](#개인정보-보호)
19. [검증 명령어](#검증-명령어)
20. [한계점 및 향후 개선](#한계점-및-향후-개선)
21. [자주 나는 오류](#자주-나는-오류)
...
루트 `.env.example`을 복사해서 `.env`를 만듭니다. 프론트 포트, 백엔드 포트, Python Agent API 요청 위치를 여기서 관리합니다.
...
`AGENT_BACKEND_URL`을 비워두면 Next.js proxy가 `http://BACKEND_HOST:BACKEND_PORT`로 요청합니다.
...
| `OPENAI_MODEL` | Next.js / Python backend | 사용할 모델명입니다. 기본 예시는 `gpt-5.5`입니다. |
| `EMBEDDING_MODEL` | Python RAG | OpenAI embedding 모델명입니다. |
| `MOCK_AI` | Next.js API route | `true`이면 Next.js 자체 Mock AI 경로가 동작합니다. |
| `JOBFIT_BACKEND_MOCK` | Python backend | `true`이면 외부 OpenAI API 대신 로컬 fallback을 사용합니다. 제출/실제 분석은 `false` 권장입니다. |
...
| `AGENT_BACKEND_URL` | Next.js proxy | 외부 backend 주소를 직접 지정할 때 사용합니다. 비우면 `BACKEND_HOST`와 `BACKEND_PORT`를 조합합니다. |
...
1. `.\scripts\dev.ps1 frontend`로 UI를 실행합니다.
